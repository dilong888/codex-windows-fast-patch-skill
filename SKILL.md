...
- If the official client imports successfully but bootstrap fails because a decoded `.pnpm\@...` path is missing while the same runtime directory exists with a `%40...` name, run `scripts\install-computer-use-local.ps1 -RuntimeAliasRepairOnly`. It adds only missing decoded directory junctions inside the active LocalAppData CUA runtime and refuses to overwrite an existing path; `-RuntimeAliasVerifyOnly` checks just those junction targets without changing them, while `-VerifyOnly` and `-StrictVerifyOnly` keep the broader plugin/browser/Chrome repair and verification scope.
- Do not treat task-top-level `nodeRepl.nativePipe` or `nodeRepl.config` being unavailable as proof that Computer Use is broken. Import the official `computer-use-client.mjs` first; its trusted import context validates the protected bridge before `setupComputerUseRuntime` is called.
- For a post-repair UI smoke test, launch and capture a new Calculator window. Do not use Windows 11 Notepad as a supposedly blank target because it can reuse an existing multi-tab window and expose another document.
